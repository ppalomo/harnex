...
- [ ] 5.4 Implement the undercut check — a project `allow` entry equal to or a prefix generalisation of a floor entry, with anything else broad reported as broad — and verify it catches `Bash(git:*)` against a floor entry for `git push`, catches a bare tool entry, and does not report an unrelated allow
